...
// If the recursive call returns false, the function backtracks by resetting the assignment 
// for the character at index n and marking its digit as unused (use[i] = 0).
//  It then continues to explore other possible assignments for the current character.
...
bool permutation(const int count, node* nodeArr, int n, string s1, string s2, string s3)
{
	...
	
	{
		...
		for (int i = 0; i < 10; i++)
		{
			...
			// if not used
			if (use[i] == 0)// use is vector of 10 ints
			{

				// assign char at index n integer i

                // for first iteration, i and n are 7, as latest funciton call is taken
				// last char will get new digit till loop ends
				nodeArr[n].v = i;
	...
	for (int i = 0; i < 10; i++)
	{
		...
		if (use[i] == 0)
		{
			...
			// different character will get this digit
			use[i] = 0;
		}
	}
		// this will bring next function call and change values from last
		return false;
}
