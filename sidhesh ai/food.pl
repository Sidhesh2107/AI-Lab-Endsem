...
asian :- verify(is_asian). 
eastern :- verify(has_meat), !. 
eastern :- verify(contain_sauce), 
           verify(not_spicy). 
 
rice :- verify(dry_texture), 
                    verify(heavy_portion), 
                    verify(need_to_be_cooked). 
noodle :- asian, verify(stringlike_shape), verify(heavy_portion),!. 

western :- verify(sweet_n_savoury). 
western :- verify(contain_sauce),verify(contain_veggies). 
...
ask(Question) :- 
        ...
        /*assert add question we yessed in working memory*/
         -> assert(yes(Question)) ;  
